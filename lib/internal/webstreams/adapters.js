      ...
      PromisePrototypeThen(
        ...
        () => {
          return PromisePrototypeThen(
            PromiseAll(
              ArrayPrototypeMap(chunks, (data) => writer.write(data.chunk))
            ),
            done,
            done
          );
...
function newReadableStreamFromStreamReadable(
  ...
  // Use queue strategy if there's any.
...
function newStreamDuplexFromReadableWritablePair(
  ...
  const duplex = new Duplex({
    ...
    destroy(error, callback) {
      ...
      if (!writableClosed || !readableClosed) {
        PromisePrototypeThen(
          PromiseAll([closeWriter(), closeReader()]),
          done,
          done
        );
        return;
  ...
  PromisePrototypeThen(
    ...
    () => {
      readableClosed = true;
      if (!isReadableEnded(duplex)) duplex.push(null);
    },
    (error) => {
      writableClosed = true;
      readableClosed = true;
...
function newWritableStreamFromStreamBase(streamBase, strategy) {
  ...
  function doWrite(chunk, controller) {
    ...
    if (ret !== 0) promise.reject(errnoException(ret, 'write', req));
